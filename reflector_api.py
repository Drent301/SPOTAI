...
def log_cost(model, input_tokens, output_tokens):
    """Logt de kosten van een GPT call."""
    costs = {
        "gpt-4o": (0.01, 0.03),
        "gpt-5": (0.02, 0.06), 
    }
...
async def handle_reflection(request: Request):
    """Ontvangt data van de Pi, stuurt naar GPT, en stuurt advies terug."""
    if not client:
    ...
    try:
        ...
        # Systeemprompt (Aangepast voor set_config test)
        system_prompt = """
        ...
        # *** HIER WAS DE FOUT - nu gecorrigeerd ***
        return {"status": "success", "reflection": gpt_result_json, "latency_ms": latency * 1000}
